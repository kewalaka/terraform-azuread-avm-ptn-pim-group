locals {
  ...
  assigned_roles_azapi = {
    for k, v in var.assigned_roles : k => {
      name  = coalesce(v.name, random_uuid.assigned_role_name[k].result)
      scope = v.scope
      body = {
        properties = {
          principalId                        = coalesce(try(v.principal_id, null), azuread_group.this.object_id)
          roleDefinitionId                   = local.assigned_roles_role_definition_resource_ids[k]
  ...
  assigned_roles_is_id = {
    for key, cfg in var.assigned_roles :
    key => (
  ...
  assigned_roles_role_definition_resource_ids = {
    for k, v in var.assigned_roles : k => (
      local.assigned_roles_is_id[k] ? v.role_definition_id_or_name : lookup(local.role_definitions_map, v.role_definition_id_or_name, v.role_definition_id_or_name)
    )
  }
  # Role assignments (permanent) - determine if the provided role_def is an ID
  assigned_roles_type = "Microsoft.Authorization/roleAssignments@2022-04-01"
